...
export async function POST(req: Request) {
  try {
    ...
    const risk = evaluateWalletTx({ tx: { type, usd: amount, counterparty: destination }, limits: store.limits, emergency: store.emergency, allowlistHit: type === "deposit" ? true : allowlistHit });
    const tx: WalletTx = { id: store.id("wtx"), walletId, type, asset, amount, usd: amount, counterparty: destination || undefined, status: risk.blocked ? "blocked" : "pending", createdAt: store.now() };
    store.walletTxs.unshift(tx);
    record(session.email, "user", risk.blocked ? "WALLET_TX_BLOCKED" : "WALLET_TX_SUBMITTED", `wallet:${walletId}`, { type, asset, amount, blocked: risk.blocked });
